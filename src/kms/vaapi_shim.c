...
void kmsvnc_vaapi_close(struct kmsvnc_vaapi *ctx) {
    ...
    if (ctx->vpp_surface != VA_INVALID_ID) {
        vaDestroySurfaces(ctx->dpy, &ctx->vpp_surface, 1);
    }
...
struct kmsvnc_vaapi *kmsvnc_vaapi_open(
    ...
    ctx->out_width = out_width ? out_width : width;
    ctx->out_height = out_height ? out_height : height;
    ctx->use_vpp_scale = (ctx->out_width != width || ctx->out_height != height);

    if (ctx->use_vpp_scale) {
        VAEntrypoint entrypoints[16];
        int num_entrypoints = 0;
        int has_vpp = 0;
        if (va_ok(
        ...
        if (!has_vpp) {
            set_errorf("VAAPI VPP is unavailable for scaled capture", NULL);
            kmsvnc_vaapi_close(ctx);
            return NULL;
        }
        if (!va_ok(
        ...
        out_attrs[0].flags = VA_SURFACE_ATTRIB_SETTABLE;
        out_attrs[0].value.type = VAGenericValueTypeInteger;
        out_attrs[0].value.value.i = selected->fourcc;
        out_attrs[1].type = VASurfaceAttribUsageHint;
        out_attrs[1].flags = VA_SURFACE_ATTRIB_SETTABLE;
        out_attrs[1].value.type = VAGenericValueTypeInteger;
        out_attrs[1].value.value.i = VA_SURFACE_ATTRIB_USAGE_HINT_VPP_WRITE;
        ...
        if (!va_ok(
                vaCreateSurfaces(
                    ...
                    out_attrs,
                    2),
                "vaCreateSurfaces(VPP scaled output)")) {
            kmsvnc_vaapi_close(ctx);
            return NULL;
        }
        if (!va_ok(
    ...
    if (!va_ok(
            vaCreateImage(ctx->dpy, selected, ctx->out_width, ctx->out_height, &ctx->image),
            "vaCreateImage")) {
        kmsvnc_vaapi_close(ctx);
        return NULL;
    }
    ...
    fprintf(stderr, "kmsvnc vaapi init fourcc=%u pitch=%u bgra=%d src=(%u,%u) size=%ux%u out=%ux%u fb=%ux%u\n",
...
int kmsvnc_vaapi_capture(struct kmsvnc_vaapi *ctx, uint8_t *dst, size_t dst_len) {
    ...
    size_t needed = (size_t)ctx->out_width * (size_t)ctx->out_height * 4;
    if (dst_len < needed) {
    ...
    VASurfaceID image_surface = ctx->surface;
    ...
    if (ctx->use_vpp_scale) {
        ...
        VARectangle dst_region = {
            .x = 0,
            .y = 0,
            .width = (uint16_t)ctx->out_width,
            .height = (uint16_t)ctx->out_height,
        };
        VAProcPipelineParameterBuffer params;
        memset(&params, 0, sizeof(params));
        params.surface = ctx->surface;
        params.surface_region = &src_region;
        params.output_region = &dst_region;
        ...
        if (!va_ok(vaBeginPicture(ctx->dpy, ctx->vpp_context, ctx->vpp_surface), "vaBeginPicture(VPP)")) {
            vaDestroyBuffer(ctx->dpy, pipeline_buf);
            return 0;
        }
        if (!va_ok(vaRenderPicture(ctx->dpy, ctx->vpp_context, &pipeline_buf, 1), "vaRenderPicture(VPP)")) {
            vaDestroyBuffer(ctx->dpy, pipeline_buf);
            return 0;
        }
        if (!va_ok(vaEndPicture(ctx->dpy, ctx->vpp_context), "vaEndPicture(VPP)")) {
            vaDestroyBuffer(ctx->dpy, pipeline_buf);
            return 0;
        }
        vaDestroyBuffer(ctx->dpy, pipeline_buf);
        image_surface = ctx->vpp_surface;
    }
    ...
    uint32_t row_bytes = ctx->out_width * 4;

    for (uint32_t y = 0; y < ctx->out_height; y++) {
        uint8_t *src_row = src + (size_t)y * src_pitch;
        uint8_t *dst_row = dst + (size_t)y * row_bytes;
        if (ctx->output_is_bgra) {
            memcpy(dst_row, src_row, row_bytes);
        } else {
            for (uint32_t x = 0; x < ctx->out_width; x++) {
